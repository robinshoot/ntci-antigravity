      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div className="space-y-4 flex-1">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 pt-2 text-xs text-slate-300">
                ...
                <div className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-[#0B0E14] border border-[#222C3D]">
                  ...
                  {isMemberLoggedIn ? (
                    ...
                    <span className="text-slate-400 font-mono flex items-center space-x-1" title="Login anggota untuk melihat nomor telepon">
                      <span>+62 812-****-****</span>
              ...
              <div className="pt-2 flex flex-wrap items-center justify-center md:justify-start gap-3">
                ...
                >
                  ...
                  {!isMemberLoggedIn && <Lock className="w-3.5 h-3.5 text-black/70 ml-1" />}
                </button>
                ...
                {isMemberLoggedIn ? (
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 px-5 py-2.5 rounded-xl text-xs font-bold text-slate-300 bg-[#1C2433] hover:bg-[#222C3D] border border-[#222C3D] transition-all cursor-pointer"
                  >
                    <Lock className="w-4 h-4 text-[#E5C158]" />
                    <span>Login untuk Kontak WA</span>
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-[#0B0E14] border border-[#222C3D] flex justify-between items-center">
                ...
                {isMemberLoggedIn ? (
                  ...
                  <span className="font-mono text-xs font-bold text-slate-400 flex items-center space-x-1.5 bg-[#171210] px-2.5 py-1 rounded-xl border border-[#332722]">
                    <span className="text-[#E5C158]">{member.motorPlate ? `${member.motorPlate.substring(0, 2)} **** ***` : 'B **** ***'}</span>
                    <span className="px-1.5 py-0.5 rounded bg-[#D4AF37]/20 text-[#E5C158] text-[9px] font-bold border border-[#D4AF37]/40 flex items-center space-x-1">
          ...
          <div className="p-6 rounded-3xl bg-[#131924] border border-[#222C3D] space-y-4 shadow-xl flex flex-col justify-between">
                ...
                <div className="p-4 rounded-2xl bg-[#0B0E14] border border-[#222C3D] space-y-1">
                  ...
                  {isMemberLoggedIn ? (
                    ...
                    <p className="text-xs text-slate-400 italic flex items-center space-x-1">
                      <Lock className="w-3.5 h-3.5 text-[#E5C158] inline mr-1 shrink-0" />
                      <span>Catatan modifikasi hanya dapat dilihat oleh Anggota Terverifikasi yang sudah login.</span>
                    </p>
                  )}
                </div>
              )}
            ...
            <button
              onClick={() => (isMemberLoggedIn ? setShowEktaModal(true) : setShowLoginModal(true))}
              className="w-full py-3 rounded-2xl text-xs font-bold text-[#00E5FF] bg-[#00E5FF]/10 border border-[#00E5FF]/30 hover:bg-[#00E5FF]/20 transition-all flex items-center justify-center space-x-2 cursor-pointer"
            >
              <Printer className="w-4 h-4" />
              <span>Cetak / Download Kartu E-KTA</span>
              {!isMemberLoggedIn && <Lock className="w-3.5 h-3.5 text-[#E5C158] ml-1" />}
            </button>
          </div>
