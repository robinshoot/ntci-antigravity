...
export async function getMembersByChapter(chapterSlug: string): Promise<MemberData[]> {
  try {
    ...
    return members.map((m) => ({
      ...
      chapterName: m.chapter ? m.chapter.name : (m.isVerified && m.role === 'SUPER_ADMIN' ? 'Pengurus Pusat' : ''),
      chapterSlug: m.chapter ? m.chapter.slug : (m.isVerified && m.role === 'SUPER_ADMIN' ? 'pusat' : ''),
